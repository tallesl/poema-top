...
def _gera_amostra(modelo: Model, vocabulario: Vocabulario, texto_completo: str) -> None:
    ...
    for temperatura in [0.7, 1.0, 1.3]:
        ...
        for i in range(200):
            ...
            proximo_indice = _seleciona_caractere_predito(previsto, temperatura)
...
def _seleciona_caractere_predito(logits: np.ndarray[np.float32], temperatura: float = 1.0) -> np.int64:
